...
void iterate(int /*argc*/, char *argv[], long long int minblock, long long int maxblock) {
    ...
    while(!feof(fp)) {
        ...
        if(block < minblock){
            //skip this block by seeking behind it and continuing the loop at the beginning
        ...
        block++;
        if(maxblock != -1 && block > maxblock) //we have seen all relevant blocks
            break;
    }
